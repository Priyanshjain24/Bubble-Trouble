...
public:
    ...
    void nextStep(double t)
    {
        ...
        double new_y = circle.getY() + vy*t + 9*t*t ;    // S = (v+u/2)*t
        ...
        else if ((vy < 0 && new_y < ( 0 + circle.getRadius()))   // bounce along y direction at upper boundary
            ||
            (vy > 0 && new_y > ( 450 - circle.getRadius())))    // bounce along y direction at lower boundary
        {

            vy = -vy ;
            new_y = circle.getY() + vy*t + 9*t*t ;
    ...
    void reflect_bubble_rect()
    {
        // reflect along x direction
        vx = - vx ;
    }

    void reflect_bubble_head()
    {
        // reflect along x direction
        vx = -vx ;
